...
PRIVATE m_dupr:={},m_pos:={},m_pag:={},m_cax:={},mcod_cli,cons_dupr:={},cons_cli:={}
...
WHILE .T.
        ...
        ************************************************************
        cComm  := "SELECT * FROM saccred WHERE venc  BETWEEN "+sr_cdbvalue(mdata1)+" AND "+sr_cdbvalue(mdata2)+ " AND datpag IS NULL"
        IF ! EMPTY(mcod_cli)
        ...
        i:=0
        FOR i = 1 TO LEN(aret)
                AADD(m_dupr,aret[i,1]+' '+STR(aret[i,7],5)+' '+aret[i,28]+' '+aret[i,2]+' '+aret[i,4]+' '+DTOC(aret[i,11])+' '+TRANSFORM(aret[i,19],'999,999.99')+'                                 ')
       ...
       NEXT
        ...
        WHILE .T.
                ...
                setcor(1)
                setcor(10);mpoint := ACHOICE(li+3,ci+1,la-3,ca-1,m_dupr,,,mponto+1);setcor(1)
                DO CASE
                        CASE LASTKEY() = 13
                                IF m_pos[mpoint,2] = 'X'
        ...
        FOR i = 1 TO LEN(m_dupr)
                ...
                IF m_pos[i,2] <> 'X'
                        LOOP
                ENDIF
                AADD(m_pag,STR(aret[i,7],5)+' '+aret[i,2]+aret[i,4]+' '+DTOC(aret[i,10])+' '+DTOC(aret[i,11])+' '+TRANSFORM(aret[i,19],'99,999.99')+' '+TRANSFORM(0,'9999.99')+' '+TRANSFORM(aret[i,19],'99,999.99')+'  '+aret[i,16]+' *')
                AADD(m_cax,{STR(aret[i,7],5),aret[i,2],aret[i,4],aret[i,11],aret[i,19],aret[i,19],aret[i,32],aret[i,30]})
