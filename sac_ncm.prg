...
WHILE .T.
        ...
        ENDIF
        mhoras := TIME()
        mensagem('Escolha o produto e tecle <ENTER> ou <ESC> p/Sair')
...
RETURN ' '
**************** f i m **********************************
FUNCTION sac_ncm1(mtipo)
*******************
LOCAL mncm,mcest,mdescri,cons_c:={}

setcor(1)
exibi_prg('SAC_NCM1')
cons_c:={}
IF mtipo = 'A'
        op_tela(20,00,24,120,'ALTERACAO NCM/CEST',,1)
        sr_getconnection():exec("SELECT * FROM sacncm WHERE codigo = "+sr_cdbvalue(m_pos[point]),,.t.,@cons_c)
        sr_getconnection():exec('COMMIT',,.f.)
ELSE
