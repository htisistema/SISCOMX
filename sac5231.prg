...
LOCAL MPRG:='SAC5231',;
      ...
      cons_merc:={},mvlr_zero:='C',mperc,mperc_aux,m_prod_nao:={},m_prod:={},mponto:=0,mop_cab:='S',mlist_pre,;
      mtipo_ordem:='1',msint_mes:='N',mtipo_doc:='  ',mtipo_merc:=' '

PRIVATE mtraco,mtit:='',mtipo,mpag,mposicao,mcod_merc:=0,mtipo_imp,mimp_tipo:=0,marq:=SPACE(35),mgrupo,msub_grupo,cons_cli:={}
...
WHILE .T.
	...
	m_prod:={}
        ...
        DEVPOS(26,2);DEVOUT('Tipo do Produto...................:')
        //IF ver_serie() = '141287'
        ...
        @ 26,38 GET mtipo_merc PICT "9" VALID mtipo_merc $ ' ,1,2,3,4,5' WHEN men_get(0,0,'Digite [1] Produto - [2] Materia Prima - [3] Isumos - [4] Consumo - [5] Outros - Branco para TODOS')
        ...
        IF msint_mes = 'N'
                ...
                cComm  := "SELECT * FROM sacmov mov WHERE data_s_e BETWEEN "+sr_cdbvalue(mdata1)+" AND "+sr_cdbvalue(mdata2)
		...
		//cComm	  := ccomm +" AND vl_vend > 0 AND NOT num_ped = 'CUPOME' AND ( documento like 'PD%' OR  documento like 'CP%')"
                ...
                ENDIF
                IF ! EMPTY(mtipo_merc)
                        cComm  := ccomm + " AND (SELECT tipo_merc FROM sacmerc WHERE cod_merc = mov.codigo) = "+sr_cdbvalue(mtipo_merc)
                ENDIF
                IF ! EMPTY(muf)
                        cComm  := ccomm + " AND (SELECT uf FROM saccli WHERE cod_cli = mov.cod_cli) = "+sr_cdbvalue(muf)
                ENDIF
