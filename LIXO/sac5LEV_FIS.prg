...
****************************************************
		...
		ENDIF
                ...
                FOR i = 1 TO LEN(cons_merc)
                        ...
                        ENDIF
                        cons_saida:={}
                        sr_getconnection():exec("SELECT codigo,SUM(quantd) FROM SACmovnt WHERE codigo = "+sr_cdbvalue(cons_merc[i,1])+"AND (cancel is null or cancel = '') AND data_s_e  BETWEEN "+sr_cdbvalue(mdata1)+" AND "+sr_cdbvalue(mdata2)+"  GROUP BY codigo",,.t.,@cons_saida)
                        IF LEN(cons_saida) = 0
                                tot_saida := 0
                        ELSE
                        ...
                        ENDIF
                        cons_ent1:={}
                        sr_getconnection():exec("SELECT codigo,SUM(quantd) FROM SACmovnt WHERE codigo = "+sr_cdbvalue(cons_merc[i,1])+"AND ent_sai = 'E' AND (cancel is null or cancel = '') AND data_s_e  BETWEEN "+sr_cdbvalue(mdata1)+" AND "+sr_cdbvalue(mdata2)+"  GROUP BY codigo",,.t.,@cons_ent1)
                        IF LEN(cons_ent1) = 0
                                tot_ent1 := 0
                        ELSE
                        ...
                        ENDIF
                        cons_ent2:={}
                        sr_getconnection():exec("SELECT codigo,SUM(quantd) FROM SACmov WHERE codigo = "+sr_cdbvalue(cons_merc[i,1])+" AND ent_sai = 'E' AND NOT tipo = '02' and (cancel is null or cancel = '') AND data_s_e  BETWEEN "+sr_cdbvalue(mdata1)+" AND "+sr_cdbvalue(mdata2)+"  GROUP BY codigo",,.t.,@cons_ent2)
                        IF LEN(cons_ent2) = 0
                                tot_ent2 := 0
                        ELSE
