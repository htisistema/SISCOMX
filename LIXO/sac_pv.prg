...
* SOLICITAR PRE-VENDA
*******************
FUNCTION sac_pv(orc,venda,m2ped)
***************
...
LOCAL mp_venda,msele,morde,f,mvar,mtelap,mtot_ipi,telaobs,;
      ...
      mcod_tab,mcont,mtipo_preco:=' ',mpre:=' ',c_orcam,m_ped,m_pos,mcons_cli,point:=0,cons_orcam := {}
...
PRIVATE MPRG:='SAC_PV',;
        ...
        mprz,mnum_ped:=0,menvelope,mcod_cli:=0,mnome_cli,mponto:=0,mflag,no,mcod_ven,;
        msubtotal,mtot_custo:=0,mcod_aux,mcomissao:=0,mcust_real,mvendido:=0,;
        ...
        mdata_pedido,m_produto:={},mnum_orc:=0,mconvidado:='',mlinha:='',mprod_merc:={}
...
SET KEY -4 TO
set_key('cliente',,,'*')
SET KEY 303 TO alt_pv()               // Alteracao de Produtos <ALT+V>
...
op_tela(00,00,50,120,memp_resa+SPACE(50)+'EMISSAO DE PRE-VENDA'+SPACE(40)+'Terminal: '+m_indiv[1,1],,1)
no  := 'nopv'
IF ! ver_nivel(mprg,'EMISSAO DE PRE-VENDA','1245',nivel_acess,,'AMBIE')
        RELEASE ALL
        wvw_lclosewindow()
        RETURN NIL
ENDIF
...
WHILE .T.
        ...
        WHILE .T.
                ...
                IF m_indiv[1,26] = 'T'
                        ...
                        mflag := 1
                        op_tela(00,01,31,110,' A B E R T U R A   D A   P R E - V E N D A                      Informacoes para a PRE-VENDA',,1)
                        botao(0,0,7,120)
                        ...
                        WHILE .T.
                                ...
                                IF ! EMPTY(mnome_cli) .OR. ! EMPTY(mcod_cli)
                                        DEVPOS(25,24);DEVOUT(cli->cod_cli)
                ...
                IF m_indiv[1,9] <> 'V'
                        ...
                        //DEVPOS(30,00);DEVOUT('Tipo de PRE-VENDA....:')
                        ver_ven(cod_operado)
                        mnome_ven := sen->snome
                        atencao(' Operador:'+cod_operado+'-'+RTRIM(mnome_ven)+' ou <ESC> p/Abandonar')
                        IF LASTKEY() = 27
                                SET KEY -9 TO
                                RELEASE ALL
                                wvw_lclosewindow()
                        ...
                        IF m_set[1,42] = 'S'
                                ...
                                WHILE .T.
                                        ...
                                        ENDIF
                                        IF EMPTY(mnum_os)
                                                IF ver_nivel('CUSTOMERC','EMISSAO DE PRE-VENDA SEM ORDEM DE SERVICOS (O.S.)','15',nivel_acess,,'AMBIE')
                                                        EXIT
                                                ELSE
                                                        LOOP
                                                ENDIF
                                        ...
                                        *****************
                                        ...
                                        ENDIF
                                        IF oss->pag = 'P'
                                        	atencao('Esta O.S. ja foi transformada em PRE_VENDA...')
                                        	LOOP
                                        ENDIF
					mpt_os := RECNO()
                                        *****************
                                        ...
                                        *****************
		                                ...
		                                mensagem('Aguarde um momento Gerando NUMERO para o PRE_VENDA....')
                		                //sr_starttrace()
                                		lExit := .T.
		                                WHILE lExit
                		                        sr_begintransaction()
                                		        try
		                                        aret:={}
                		                        sr_getconnection():exec( "select numero from sacnopv where sr_recno = 1 for update with lock",,.t.,@aret)
                                		        mnum_ped = VAL(aret[1,1])+1
		                                        mnum = strzero(VAL(aret[1,1])+1,6)
                		                        sr_getconnection():exec("update sacnopv set numero = " + sr_cdbvalue(mnum) + ", data_ped = " + sr_cdbvalue( mdata_sis) + " where sr_recno = 1 ",,.f.)
                                		        ...
                                		        aret:={}
		                                        cComm  := "SELECT count(*) FROM sacpv WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))
                                		...
                                		sr_endtransaction()
		                                END
                		                Wvw_SetTitle( ,memp_resa+SPACE(46)+'EMISSAO DO PRE-VENDA No.:'+STRZERO(mnum_ped,10)+SPACE(30)+'Terminal: '+m_indiv[1,1])
				                        ...
				                        ENDIF
                        			        ...
                        			        sr_cdbvalue(STRZERO(mcod_cli,5)      )+','+;//24
			                                sr_cdbvalue(''                       )+','+;//25
                        			        sr_cdbvalue(''                       )+','+;//26
                        ...
                        IF (m_set[1,100] = 'S' .OR. m_set[1,10] = 'S') //.AND. m_indiv[1,26] = 'N'
                                WHILE .T.
                                        SET KEY -8 TO sac130()           // inclusao de clientes
                                        IF ! AbriArq('saccli','cli');RETURN NIL;ENDIF
                                        ...
                                        ENDIF
                                        IF ! EMPTY(mnome_cli) .OR. ! EMPTY(mcod_cli)
                                                DEVPOS(07,23);DEVOUT(cli->cod_cli)
                        ...
                        WHILE .T. .AND. m_set[1,108] = 'S'
                                ...
                                IF m_set[1,10] = 'S' .AND. mcod_ven <> mcod_aux .AND. STRZERO(mcod_cli,5) <> m_set[1,84];
                                   .AND. ! aut_sen('Cod. Vend. Diferente do Vend. Resp.CLIENTE, Senha autorizacao:','LIBCLIVEN',,STRZERO(mcod_cli,5))
                                        LOOP
                                ENDIF
                                EXIT
                        ENDDO
                        ...
                        IF m_set[1,46] = 'S'
                                WHILE .T.
                                        ...
                                        IF mcod_cond <> VAL(cli->cod_cond) .AND. VAL(cli->cod_cond) > 0
                                                IF ! aut_sen('Alteracao da Condicao de Pagamento pre fixada do CLIENTE: de '+cli->cod_cond+' para '+STRZERO(mcod_cond,3)+' - Autorizacao:','LIB_CONDPAG',,STRZERO(mcod_cli,5))
                                                        LOOP
                                                ENDIF
                                        ENDIF
                        ...
                        ENDIF
                        opcao := op_simnaocan('S','CONFIRMA TODAS AS INFORMACOES DO PRE-VENDA:')
                        IF LASTKEY() = 27 .OR. opcao = 'N'
                                wvw_lclosewindow()
                                LOOP
                        ENDIF
        ...
        Wvw_SetTitle( ,memp_resa+SPACE(50)+'EMISSAO DE PRE-VENDA'+SPACE(40)+'Terminal: '+m_indiv[1,1])
        ...
        IF m_set[1,84] <> STRZERO(mcod_cli,5) .AND. ! EMPTY(mcod_cli)
                mlimite := cli->limite
        ...
        ********************
        WHILE .T.
                botao(00,01,10,119)
                SUB_BANNER(01,05,'P R O D U T O')
                SETCOLOR('B/W,W/B')
                botao(lci-3,cci+55,lba,cba-1)
                WVW_DrawLabel(,lci-3,cci+60,'No.Pre-Venda: '+STRZERO(mnum_ped,10),,,,, 'Arial Black',35,20,,,,,)
                IF LEN(m_codigo) = 0
                        limpa(01,02,10,119)
                        IF m_indiv[1,26] = 'T'
                ...
                IF orc # NIL .AND. EMPTY(mnum_ped)
                        ...
                        ELSE
                                ...
                                FOR y = 1 TO LEN(c_orcam)
                                        ...
                                        limpa(01,01,10,119)
                                        @ 28,14 SAY SPACE(13)
                                        @ 28,14 SAY mped_merc[1,8]
                                        @ 29,14 SAY mped_merc[1,9]
                                        @ 30,14 SAY c_orcam[y,14] PICT '9,999,999.99'
                                        @ 31,14 SAY c_orcam[y,18] PICT '9,999,999.99'
                                        @ 32,14 SAY c_orcam[y,18] * c_orcam[y,14] PICT '9,999,999.99'
                                        ...
                                        WVW_DrawLabel(,08,70,TRANSFORM(mped_merc[1,46],'@E 999,999.99'),,,240,, 'arial black',60,30,,,,,)
                                        setcor(1)
                                        IF mped_merc[1,24] = 'N'
	                                ...
	                                IF c_orcam[y,14] > mped_merc[1,56] .AND. m_set[1,107] = 'S' .AND. SUBSTR(mped_merc[1,9],1,1) <> '@'
                                        ...
                                        IF c_orcam[y,18] > mped_merc[1,46]
                                                ...
                                                IF ((mped_merc[1,46] - c_orcam[y,18])/mped_merc[1,46])*100 > m_set[1,33] .AND. EMPTY(mped_merc[1,80]) .AND. ! EMPTY(m_set[1,33])
                                                        IF ! aut_sen('Codigo do Produto: '+mped_merc[1,8]+' Vlr.Solicitado R$:'+TRANSFORM(c_orcam[y,18],'999,999.99')+' - Pr.Venda R$:'+TRANSFORM(mped_merc[1,46],'999,999.99')+' - '+TRANSFORM(((mped_merc[1,46] - c_orcam[y,18])/mped_merc[1,46])*100,'999.99')+'% Desconto nao e permitido.. senha autorizacao:','LIB_DESC',,'',STRZERO(mcod_merc,5))
                                                                mquantd := 1
                                                                LOOP
                                                        ENDIF
                                                ELSEIF ((mped_merc[1,46] - c_orcam[y,18])/mped_merc[1,46])*100 > mped_merc[1,80] .AND. ! EMPTY(mped_merc[1,80])
                                                        IF ver_serie() = '141220' .OR. ver_serie() = '141350' .OR. ver_serie() = '141384'
                                                                ...
                                                                LOOP
                                                        ENDIF
                                                        IF ! aut_sen('Codigo do Produto: '+mped_merc[1,8]+' Vlr.Solicitado R$:'+TRANSFORM(c_orcam[y,18],'999,999.99')+' - Pr.Venda R$:'+TRANSFORM(mped_merc[1,20],'999,999.99')+' - '+TRANSFORM(((mped_merc[1,20] - c_orcam[y,18])/mped_merc[1,20])*100,'999.99')+'% Desconto nao e permitido.. senha autorizacao:','LIB_DESC',,'',STRZERO(mcod_merc,5))
                                                                mquantd := 1
                                                                LOOP
                                                        ENDIF
                                                ELSEIF m_set[1,38] = 'C' .AND. c_orcam[y,18] < mped_merc[1,45]
                                                        IF ! aut_sen('Cod.Produto: '+STRZERO(mcod_merc,5)+' Vlr.Solicitado R$:'+TRANSFORM(c_orcam[y,18],'999,999.99')+' - Pr.Custo R$:'+TRANSFORM(mped_merc[1,45],'999,999.99')+' - Senha de autorizacao:','LIB_DESC',,'',STRZERO(mcod_merc,5))
                                                                mquantd := 1
                                                                LOOP
                                                        ENDIF
                                                ELSEIF m_set[1,38] = 'V' .AND. c_orcam[y,18] < mped_merc[1,46]
                                                        IF ! aut_sen('Cod.Produto: '+STRZERO(mcod_merc,5)+' Vlr.Solicitado R$:'+TRANSFORM(c_orcam[y,18],'999,999.99')+' - Pr.Venda R$:'+TRANSFORM(mped_merc[1,20],'999,999.99')+' - Senha de autorizacao:','LIB_DESC',,'',STRZERO(mcod_merc,5))
                                                                mquantd := 1
                                                                LOOP
                                                        ENDIF
                                                ENDIF
                                        ...
                                        IF EMPTY(mnum_ped)
                                                ...
                                                //sr_endtransaction()
                                                END
                                                Wvw_SetTitle( ,memp_resa+SPACE(50)+'EMISSAO DO PRE-VENDA No.:'+STRZERO(mnum_ped,6)+SPACE(30)+'Terminal: '+m_indiv[1,1])
                                        ENDIF
                                        mprod_merc:={}
                                        sr_getconnection():exec("SELECT * FROM sacmerc WHERE cod_merc = "+sr_cdbvalue(c_orcam[y,6]),,.t.,@mprod_merc)
                                        sr_getconnection():exec('COMMIT',,.f.)
                                        mlinha := mprod_merc[1,2]+mprod_merc[1,8]+mprod_merc[1,9]+mprod_merc[1,14]+STRZERO((mprod_merc[1,56] - c_orcam[y,14])*1000,13)+STRZERO(iat(mprod_merc[1,46])*100,14)+mprod_merc[1,68]
                                        ...
                                        sr_getconnection():exec('INSERT INTO logproduto (data_sis,data,'+;
                                                                ...
                                                                sr_cdbvalue('INCLUSAO PV: '+STRZERO(mnum_ped,6))+','+; //11
                                                                sr_cdbvalue('S')+','+; //11
                                                                sr_cdbvalue(' ')+')',,.f.)
                                        ...
                                        *                          40      41    42      43     44
                                        ...
                                        mitem := 1
                                        cons_tela := {}
                                        sr_getconnection():exec("SELECT * FROM sacpv WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6)),,.t.,@cons_tela)
                                        sr_getconnection():exec('COMMIT',,.f.)
                                        IF LEN(cons_tela) > 20
                                        ...
                                        ENDIF
                                        @ lci+f,cci+55 SAY ''
                                        WVW_DrawLabel(,lci+f,COL()+1,'Cliente:'+cli->cod_cli+'-'+LEFT(cli->razao,35)+' - Vend.:'+mcod_ven+'-'+LEFT(mnome_ven,25),,,,, 'terminal',13,4,,,,,)
                                        f++
                ...
                FOR i = 1 TO LEN(cons_tela)
                        ...
                        IF mqtd_lin < f
                                ...
                                IF m_set[1,147] = 'C'
                                        IF cons_tela[i,8] = 'EV ' .OR. cons_tela[i,8] = 'PL '
                                                //WVW_DrawLabel(,lci+f,COL(),STRZERO(i,4)+'  '+cons_tela[i,6]+'  '+LEFT(cons_tela[i,7],10)+' '+cons_tela[i,10]+' '+cons_tela[i,80]+' '+TRANSFORM(cons_tela[i,14],ALLTRIM(m_set[1,99]))+' '+IF(cons_tela[i,106] # 1,TRANSFORM(cons_tela[i,18],ALLTRIM('@E '+m_set[1,98]))+' '+TRANSFORM(cons_tela[i,18] * cons_tela[i,14],ALLTRIM('@E '+m_set[1,98])),'    CANCELADO'),,,,, 'terminal',13,4,,,,,)
                                                WVW_DrawLabel(,lci+f,COL(),STRZERO(i,4)+'  '+cons_tela[i,6]+'  '+LEFT(cons_tela[i,7],10)+' '+cons_tela[i,10]+' '+cons_tela[i,80]+' '+TRANSFORM(cons_tela[i,14],ALLTRIM(m_set[1,99]))+' '+TRANSFORM(cons_tela[i,18],ALLTRIM('@E '+m_set[1,98]))+' '+TRANSFORM(cons_tela[i,18] * cons_tela[i,14],ALLTRIM('@E '+m_set[1,98])),,,,, 'terminal',13,4,,,,,)
                                                IF cons_tela[i,106] = 1
                                                        f++
                                                        WVW_DrawLabel(,lci+f,COL(),'ITEM CANCELADO: '+STRZERO(1,3),,,,, 'terminal',13,4,,,,,)
                                                ENDIF
                                        ELSE
                                                //WVW_DrawLabel(,lci+f,COL(),STRZERO(i,4)+'  '+cons_tela[i,6]+'  '+cons_tela[i,7]+'   '+TRANSFORM(cons_tela[i,14],ALLTRIM(m_set[1,99]))+' '+IF(cons_tela[i,106] # 1,TRANSFORM(cons_tela[i,18],ALLTRIM('@E '+m_set[1,98]))+' '+TRANSFORM(cons_tela[i,18] * cons_tela[i,14],ALLTRIM('@E '+m_set[1,98])),'    CANCELADO'),,,,, 'terminal',13,4,,,,,)
                                                WVW_DrawLabel(,lci+f,COL(),STRZERO(i,4)+'  '+cons_tela[i,6]+'  '+cons_tela[i,7]+'   '+TRANSFORM(cons_tela[i,14],ALLTRIM(m_set[1,99]))+' '+TRANSFORM(cons_tela[i,18],ALLTRIM('@E '+m_set[1,98]))+' '+TRANSFORM(cons_tela[i,18] * cons_tela[i,14],ALLTRIM('@E '+m_set[1,98])),,,,, 'terminal',13,4,,,,,)
                                                IF cons_tela[i,106] = 1
                                                        f++
                                                        WVW_DrawLabel(,lci+f,COL(),'ITEM CANCELADO: '+STRZERO(1,3),,,,, 'terminal',13,4,,,,,)
                                        ...
                                        ENDIF
                                ELSE
                                        //WVW_DrawLabel(,lci+f,COL(),STRZERO(i,4)+'  '+IF(! EMPTY(m_cbarra[i]),m_cbarra[i],STRZERO(VAL(cons_tela[i,6]),14))+'  '+LEFT(cons_tela[i,7],33)+' '+TRANSFORM(cons_tela[i,14],ALLTRIM(m_set[1,99]))+' '+IF(cons_tela[i,106] # 1,TRANSFORM(cons_tela[i,18],ALLTRIM('@E '+m_set[1,98]))+' '+TRANSFORM(cons_tela[i,18] * cons_tela[i,14],ALLTRIM('@E '+m_set[1,98])),'    CANCELADO'),,,,, 'terminal',13,4,,,,,)
                                        WVW_DrawLabel(,lci+f,COL(),STRZERO(i,4)+'  '+IF(! EMPTY(m_cbarra[i]),m_cbarra[i],STRZERO(VAL(cons_tela[i,6]),14))+'  '+LEFT(cons_tela[i,7],33)+' '+TRANSFORM(cons_tela[i,14],ALLTRIM(m_set[1,99]))+' '+TRANSFORM(cons_tela[i,18],ALLTRIM('@E '+m_set[1,98]))+' '+TRANSFORM(cons_tela[i,18] * cons_tela[i,14],ALLTRIM('@E '+m_set[1,98])),,,,, 'terminal',13,4,,,,,)
                                        IF cons_tela[i,106] = 1
                                                f++
                                                WVW_DrawLabel(,lci+f,COL(),'ITEM CANCELADO: '+STRZERO(1,3),,,,, 'terminal',13,4,,,,,)
                ...
                SET KEY -9 TO fecha_pv('1')       // neste mesmo PRG. <F10>Tecla
                //mensagem('Digite codigo ou Descricao - <F10>Fechamento <ALT+V>Alt.Prod. <PD>2o.Pedido ou <OR>2o.Orcamento <PRE>Lista de Presente <ESC>Abandonar')
                mensagem('<F10>Fechamento <ALT>Alterar Produto - <X mais a qtd.>Informa Qtd. - <PRE>Lista Presente - <TAB>Alterar Cond.Pagamento - <ESC>Abandonar')
                WVW_DrawBoxGet(,20,14,LEN(mcod_bc))
                SET KEY -7 TO F8_PROD()
                mloop := 0
                @ 28,14 GET mcod_bc PICT '@!' WHEN WVW_DrawBoxGet(,28,14,LEN(mcod_bc))
                READ
                IF ALLTRIM(mcod_bc) = 'ORCAMENTO'
                        IF LEN(m_codigo) > 0
                                atencao('Nao e possivel Incluir este orcamento neste PRE-VENDA porque ja foi Incluso algum Produto...')
                                LOOP
                        ENDIF
                        mnum_orc := 0
                        ...
                        IF LEN(c_orcam) = 0
                                ...
                                LOOP
                        ENDIF
                        opcao := op_simnao('S','Confirma o ORCAMENTO para PRFE-VENDA:')
                        IF LASTKEY() = 27 .OR. opcao = 'N'
                                wvw_lclosewindow()
                                LOOP
                        ENDIF
...
*/
                                mprod_merc:={}
                                sr_getconnection():exec("SELECT * FROM sacmerc WHERE cod_merc = "+sr_cdbvalue(c_orcam[i,6]),,.t.,@mprod_merc)
                                sr_getconnection():exec('COMMIT',,.f.)
                                mlinha := mprod_merc[1,2]+mprod_merc[1,8]+mprod_merc[1,9]+mprod_merc[1,14]+STRZERO((mprod_merc[1,56] - c_orcam[i,14])*1000,13)+STRZERO(iat(mprod_merc[1,46])*100,14)+mprod_merc[1,68]
                                sr_getconnection():exec("UPDATE sacmerc SET saldo_mer = "+sr_cdbvalue(mprod_merc[1,56] - c_orcam[i,14])+",data_atu = "+sr_cdbvalue(mdata_sis)+",chv_cript = "+sr_cdbvalue(CRIPTO(mlinha,1))+" WHERE cod_merc = "+sr_cdbvalue(c_orcam[i,6]),,.f.)
                                sr_getconnection():exec('INSERT INTO logproduto (data_sis,data,'+;
                                                        'hora,cod_prod,quantd,saldo_ant,saldo_pos,cod_oper,prog,terminal,'+;
                                                        'processo,ent_sai,SR_DELETED )'+;
                                                        ' VALUES ('+;
                                                        ...
                                                        sr_cdbvalue(mprod_merc[1,56])+','+; //6
                                                        sr_cdbvalue(mprod_merc[1,56] - c_orcam[i,14])+','+; //7
                                                        sr_cdbvalue(cod_operado)+','+; //8
                                                        sr_cdbvalue('SAC23PD1')+','+; //9
                                                        sr_cdbvalue(LEFT(NETNAME(),15))+','+; //12
                                                        sr_cdbvalue('INCLUSAO PV: '+STRZERO(mnum_ped,6))+','+; //11
			...
			mcod_bc := SPACE(14)
                        IF LEN(m_codigo) > 0
                                atencao('Nao e possivel Incluir este orcamento nesta PRE-VENDA porque ja foi Incluso algum Produto...')
                                LOOP
                        ENDIF
			...
			IF ! EMPTY(mcod_oper)
				...
				mcons_cli := {}
				point := 0
        	                sr_getconnection():exec("SELECT pnum_ped,pcod_cli,sum(pquantd*pvlr_fat),pdat_ped FROM sacpv WHERE pcod_oper = "+sr_cdbvalue(STRZERO(mcod_oper,3))+" AND pfecha IS NULL AND pdatapag IS NULL GROUP BY pnum_ped,pnum_ped,pcod_cli,pcod_cli,pdat_ped,pdat_ped ORDER BY pnum_ped",,.t.,@c_orcam)
                                sr_getconnection():exec('COMMIT',,.f.)
                	        IF LEN(c_orcam) = 0
                        	        atencao('Nao foi possivel encontrar este nenhum PRE-VENDA...')
                                	wvw_lclosewindow()
	                                LOOP
        	                ENDIF
				...
				FOR i = 1 to LEN(c_orcam)
					mcons_cli := {}
					sr_getconnection():exec("SELECT * FROM saccli WHERE cod_cli = "+sr_cdbvalue(c_orcam[i,2]),,.t.,@mcons_cli)
                                        sr_getconnection():exec('COMMIT',,.f.)
					IF LEN(mcons_cli) = 0
			        	        AADD(m_ped,DTOC(c_orcam[i,4])+'  '+c_orcam[i,1]+'  '+TRANSFORM(c_orcam[i,3],'999,999.99')+'  00000 - ')
		                        	AADD(m_pos,c_orcam[i,1])
					ELSE
			        	        AADD(m_ped,DTOC(c_orcam[i,4])+'  '+c_orcam[i,1]+'  '+TRANSFORM(c_orcam[i,3],'999,999.99')+'  '+mcons_cli[1,2]+' - '+mcons_cli[1,3])
		                        	AADD(m_pos,c_orcam[i,1])
					ENDIF		
			        NEXT
			...
			ENDIF
        	       	c_orcam := {}
                        sr_getconnection():exec("SELECT * FROM sacpv WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_orc,6)),,.t.,@c_orcam)
                        sr_getconnection():exec('COMMIT',,.f.)
	                IF LEN(c_orcam) = 0
	        	        atencao('Nao foi possivel encontrar este PRE-VENDA...')
        	        	wvw_lclosewindow()
                	        LOOP
	                ENDIF
			IF ! EMPTY(c_orcam[1,100])
				atencao('Este PRE-VENDA ja foi FECHADO Faca um ALTERACAO no mesmo ....')
        	        	wvw_lclosewindow()
				LOOP
			ENDIF
                        opcao := op_simnao('S','Confirma o PRE-VENDA de No. '+STRZERO(mnum_orc,6)+':')
                        IF LASTKEY() = 27 .OR. opcao = 'N'
                                wvw_lclosewindow()
                                LOOP
                        ENDIF
			...
			wvw_lclosewindow()
                        mcod_cli := VAL(c_orcam[1,23])
			mcons_cli := {}
			sr_getconnection():exec("SELECT * FROM saccli WHERE cod_cli = "+sr_cdbvalue(c_orcam[1,23]),,.t.,@mcons_cli)
                        sr_getconnection():exec('COMMIT',,.f.)
			IF LEN(mcons_cli) > 0
	                        mcod_aux  := mcons_cli[1,106]
        	                mcod_ven := mcons_cli[1,106]
                	        mcod_cond := VAL(mcons_cli[1,133])
                        	IF mcons_cli[1,45] = 'A'
	                        	mvarejo := 2
		               	ELSE
                	        	mvarejo := 1
			...
			ENDIF
                ...
                IF SUBSTR(mcod_bc,1,1) = '*'
                        ...
                        //mcod_merc := IF(EMPTY(m_cfg[88])  ,f4_merc(,mcod_bc,STRZERO(mvarejo,1),mperc),f4_merc1(,mcod_bc,STRZERO(mvarejo,1),mperc))
                ...
                IF LASTKEY() = 27
                        IF LEN(m_codigo) = 0 .AND. mnum_ped = 0
        	                ...
        	                SET KEY -7 TO
                                ...
                                EXIT
                        ENDIF
			IF 'N' = op_simnao('S','Deseja Abandonar o PRE-VENDA:')
				LOOP
			ENDIF	
                        SET KEY -2 TO
                        SET KEY -7 TO
                ...
                IF ! EMPTY(mcod_bc)
                        IF mcod_bc = 'TAB'
                                ...
                                m_matriz := {}
                                c_ped := {}
                                sr_getconnection():exec("SELECT * FROM sacpv WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6)),,.t.,@c_ped)
                                sr_getconnection():exec('COMMIT',,.f.)
                                FOR mcont = 1 TO LEN(c_ped)
                                        ...
                                        IF ! EMPTY(cons_tabpg[1,24])
                                                IF cons_tabpg[1,24] = '1'
                                                        ccomm :="UPDATE sacpv SET pvlr_fat = "+sr_cdbvalue(m_produto[1,46])+",ppre_venda = "+sr_cdbvalue(m_produto[1,46])+",ppre_dig = "+sr_cdbvalue(m_produto[1,46])  +" WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(c_ped[mcont,6])
                                                        mvlr_fat := m_produto[1,46]
                                                ELSEIF cons_tabpg[1,24] = '2'
                                                        ccomm :="UPDATE sacpv SET pvlr_fat = "+sr_cdbvalue(m_produto[1,47])+",ppre_venda = "+sr_cdbvalue(m_produto[1,47])+",ppre_dig = "+sr_cdbvalue(m_produto[1,47])  +" WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(c_ped[mcont,6])
                                                        mvlr_fat := m_produto[1,47]
                                                ELSEIF cons_tabpg[1,24] = '3'
                                                        ccomm :="UPDATE sacpv SET pvlr_fat = "+sr_cdbvalue(m_produto[1,100])+",ppre_venda = "+sr_cdbvalue(m_produto[1,100])+",ppre_dig = "+sr_cdbvalue(m_produto[1,100])  +" WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(c_ped[mcont,6])
                                                        mvlr_fat := m_produto[1,100]
                                                ELSEIF cons_tabpg[1,24] = '4'
                                                        ccomm :="UPDATE sacpv SET pvlr_fat = "+sr_cdbvalue(m_produto[1,101])+",ppre_venda = "+sr_cdbvalue(m_produto[1,101])+",ppre_dig = "+sr_cdbvalue(m_produto[1,101])  +" WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(c_ped[mcont,6])
                                                        mvlr_fat := m_produto[1,101]
                                                ELSE
                                                        ccomm :="UPDATE sacpv SET pvlr_fat = "+sr_cdbvalue(m_produto[1,102])+",ppre_venda = "+sr_cdbvalue(m_produto[1,102])+",ppre_dig = "+sr_cdbvalue(m_produto[1,102])  +" WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(c_ped[mcont,6])
                                        ...
                                        ELSE
                                                IF cons_tabpg[1,5] =  0
                                                        ...
                                                        mvlr_fat := m_produto[1,46]
                                                ELSE
                                                        ccomm :="UPDATE sacpv SET pvlr_fat = "+sr_cdbvalue(m_produto[1,46] *  ((cons_tabpg[1,5]/100)+1))+",ppre_venda = "+sr_cdbvalue(m_produto[1,46] *  ((cons_tabpg[1,5]/100)+1))+",ppre_dig = "+sr_cdbvalue(m_produto[1,46] *  ((cons_tabpg[1,5]/100)+1)) +" WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(c_ped[mcont,6])
                        ...
                        ELSEIF LEN(RTRIM(mcod_bc)) > 5 .OR. VAL(mcod_bc) = 0
                                ...
                                IF LEN(mped_merc) > 0
                                        m_produto := {}
                                        sr_getconnection():exec("SELECT * FROM sacpv WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(mped_merc[1,8]),,.t.,@m_produto)
                                        sr_getconnection():exec('COMMIT',,.f.)
                                        IF LEN(m_produto) > 0 .AND. RTRIM(mped_merc[1,9]) <> 'DIVERSOS' .AND. m_set[1,104] <> 'S' .AND. m_indiv[1,26] <> 'T'
                                                alt_pv(mped_merc[1,8])
                                                LOOP
                                        ENDIF
                                ELSE
                                        mped_merc := {}
                        ...
                        ELSE
                                mcod_merc := VAL(mcod_bc)
                                m_produto := {}
                                sr_getconnection():exec("SELECT * FROM sacpv WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6))+" AND pcod_merc = "+sr_cdbvalue(STRZERO(mcod_merc,5)),,.t.,@m_produto)
                                sr_getconnection():exec('COMMIT',,.f.)
                                IF LEN(m_produto) > 0 .AND. RTRIM(m_produto[1,9]) <> 'DIVERSOS' .AND. m_set[1,104] <> 'S' .AND. m_indiv[1,26] <> 'T'
                                        alt_pv(STRZERO(mcod_merc,5))
                                        LOOP
                                ENDIF
                                mped_merc := {}
                                sr_getconnection():exec("SELECT * FROM sacmerc WHERE cod_merc = "+sr_cdbvalue(STRZERO(mcod_merc,5)),,.t.,@mped_merc)
                ...
                IF FILE(ALLTRIM(m_indiv[1,29])+'P'+mped_merc[1,8]+'.jpg')
                        limpa(12,01,26,53)
                        botao(12,01,26,53)
                        WVW_DrawImage( ,12,01,26,53,ALLTRIM(m_indiv[1,29])+'P'+mped_merc[1,8]+'.jpg',.T.,.F.)
                ELSEIF FILE(ALLTRIM(m_indiv[1,29])+'P'+mped_merc[1,8]+'.bmp')
                        limpa(12,01,26,53)
                        botao(12,01,26,53)
                        WVW_DrawImage( ,12,01,26,53,ALLTRIM(m_indiv[1,29])+'P'+mped_merc[1,8]+'.bmp',.T.,.F.)
                ELSEIF FILE(ALLTRIM(m_indiv[1,3])+'\PRODUTO\P'+mped_merc[1,8]+'.jpg')
                        limpa(12,01,26,53)
                        botao(12,01,26,53)
                        WVW_DrawImage( ,12,01,26,53,ALLTRIM(m_indiv[1,3])+'\PRODUTO\P'+mped_merc[1,8]+'.jpg',.T.,.F.)
                ELSEIF FILE(ALLTRIM(m_indiv[1,3])+'\PRODUTO\P'+mped_merc[1,8]+'.bmp')
                        limpa(12,01,26,53)
                        botao(12,01,26,53)
                        WVW_DrawImage( ,12,01,26,53,ALLTRIM(m_indiv[1,3])+'\PRODUTO\P'+mped_merc[1,8]+'.bmp',.T.,.F.)
                ...
                IF m_set[1,125] = 'S' .AND. mped_merc[1,23] = 0
                        ...
                        IF (mdesc_prod > mped_merc[1,80] .AND. ! EMPTY(mped_merc[1,80]));
                           .OR. (mdesc_prod > m_set[1,33] .AND. EMPTY(mped_merc[1,80]))
                                IF ! aut_sen(TRANSFORM(mdesc_prod,"999.99")+"% Desconto nao e permitido.. senha autorizacao:","LIB_DESC",,,STRZERO(mcod_merc,5))
                                        setcor(1)
                                        mquantd := 1
                                        LOOP
                                ENDIF
		...
		DEVPOS(28,30);DEVOUT(IF(! EMPTY(mped_merc[1,23]),'PROMOCAO:'+TRANSFORM(mped_merc[1,23],'99')+'%',''))
                ...
                IF mped_merc[1,14] = 'EV'
                        ...
                        @ 01,22 GET menvelope PICT '@!' VALID IF(EMPTY(menvelope),.F.,ver_envelope(menvelope+STRZERO(mcod_cli,5)))
                        READ
                        wvw_lclosewindow()
                        IF LASTKEY() = 27
                                LOOP
                ...
                ELSEIF mped_merc[1,14] = 'PL '
                        ...
                        @ 01,13 GET menvelope PICT '@! XXX-9999' VALID IF(EMPTY(menvelope),.F.,.T.)
			...
			DEVPOS(31,27);DEVOUT('Vlr.do IPI:'+TRANSFORM(mvlr_fat*((mped_merc[1,65]/100)),ALLTRIM('@E '+m_set[1,98])))
		ELSE
			DEVPOS(31,27);DEVOUT('                      ')
		ENDIF
                ...
                IF SUBSTR(mcod_bc,1,1) = '2' .AND. (LEN(RTRIM(mcod_bc)) > 5 .OR. VAL(mcod_bc) = 0)
                        ...
                        ENDIF
                        DEVPOS(30,14);DEVOUTPICT(mquantd,ALLTRIM(m_set[1,99]))
                        DEVPOS(31,14);DEVOUTPICT(mvlr_fat,ALLTRIM('@E '+m_set[1,98]))
                        mcomissao := mped_merc[1,26]
                        setcor(1)
                ELSEIF m_indiv[1,26] <> 'T' .OR. mped_merc[1,25] = 'S' .OR. mvlr_fat = 0
                        ...
                        WVW_DrawBoxGet(,30,14,LEN(mmasc_qtd))
                        ...
                        IF mquantd > mped_merc[1,56] .AND. mped_merc[1,9] <> 'DIVERSOS' .AND. m_set[1,107] = 'S' .AND. SUBSTR(mped_merc[1,9],1,1) <> '@'
                                IF ! aut_sen('Quantidade Solicitada: '+ALLTRIM(TRANSFORM(mquantd,'999,999,999.99'))+' maior que saldo: '+ALLTRIM(TRANSFORM(mped_merc[1,56],'999,999.99'))+' ... Senha de autorizacao:','LIB_SALDO',,'',STRZERO(mcod_merc,5))
                                        LOOP
                                ENDIF
                                mlibera := 'S'
                        ENDIF
                        IF ! EMPTY(m_set[1,153]) .AND. mquantd > m_set[1,153]
                                IF ! aut_sen('Quantidade Solicitada: '+ALLTRIM(TRANSFORM(mquantd,'999,999,999.99'))+' maior que Permitido pela ADM: '+ALLTRIM(TRANSFORM(m_set[1,153],'999,999.99'))+' ... Senha de autorizacao:','LIB_SALDOADM',,'',STRZERO(mcod_merc,5))
                                        LOOP
                                ENDIF
                        ENDIF
                        IF mped_merc[1,23] > 0
...
*/                                
                        ...
                        WVW_DrawBoxGet(,31,14,LEN(m_set[1,98]))
                        ...
                        IF ! EMPTY(m_set[1,154]) .AND. mvlr_fat > m_set[1,154]
                                IF ! aut_sen('Cod.Produto: '+STRZERO(mcod_merc,5)+' - Valor por Produto Solicitada: '+ALLTRIM(TRANSFORM(mvlr_fat,'999,999,999.99'))+' maior que Permitido pela ADM: '+ALLTRIM(TRANSFORM(m_set[1,154],'999,999.99'))+' ... Senha de autorizacao:','LIB_SALDO',,'',STRZERO(mcod_merc,5))
                                        LOOP
                                ENDIF
                        ENDIF
                        mcomissao := mped_merc[1,26]
                        IF mp_venda > mvlr_fat
                                ...
                                IF ((mp_venda - mvlr_fat)/mp_venda)*100 > m_set[1,33] .AND. EMPTY(mped_merc[1,80]) .AND. ! EMPTY(m_set[1,33])
                                        IF ! aut_sen('Codigo do Produto: '+mped_merc[1,8]+' Vlr.Solicitado R$:'+TRANSFORM(mvlr_fat,'999,999.99')+' - Pr.Venda R$:'+TRANSFORM(mp_venda,'999,999.99')+' - '+TRANSFORM(((mp_venda - mvlr_fat)/mp_venda)*100,'999.99')+'% Desconto nao e permitido.. senha autorizacao:','LIB_DESC',,'',STRZERO(mcod_merc,5))
                                                mquantd := 1
                                                LOOP
                                        ENDIF
                                ...
                                ELSEIF m_set[1,38] = 'C' .AND. mvlr_fat < mped_merc[1,45]
                                        IF ! aut_sen('Cod.Produto: '+STRZERO(mcod_merc,5)+' Vlr.Solicitado R$:'+TRANSFORM(mvlr_fat,'999,999.99')+' - Pr.Custo R$:'+TRANSFORM(mped_merc[1,45],'999,999.99')+' - Senha de autorizacao:','LIB_DESC',,'',STRZERO(mcod_merc,5))
                                                mquantd := 1
                                                LOOP
                                        ENDIF
                                ELSEIF m_set[1,38] = 'V' .AND. mvlr_fat < mp_venda
                                        IF ! aut_sen('Cod.Produto: '+STRZERO(mcod_merc,5)+' Vlr.Solicitado R$:'+TRANSFORM(mvlr_fat,'999,999.99')+' - Pr.Venda R$:'+TRANSFORM(mp_venda,'999,999.99')+' - Senha de autorizacao:','LIB_DESC',,'',STRZERO(mcod_merc,5))
                                                mquantd := 1
                                                LOOP
                                        ENDIF
                                ENDIF
	        ...
	        */
                                ...
                                //sr_endtransaction()
                                END
                                Wvw_SetTitle( ,memp_resa+SPACE(46)+'EMISSAO DO PRE-VENDAS No.:'+STRZERO(mnum_ped,10)+SPACE(30)+'Terminal: '+m_indiv[1,1])
                        ENDIF
                        aret:={}
                        ...
                        IF ! (aret[1,11] = 'S' .AND. (LEN(RTRIM(mcod_bc)) > 5 .OR. VAL(mcod_bc) = 0))
                                IF mquantd > aret[1,56] .AND. aret[1,9] <> 'DIVERSOS' .AND. m_set[1,107] = 'S' .AND. mlibera = ' ' .AND. SUBSTR(aret[1,9],1,1) <> '@'
                                        IF ! aut_sen('Sld:'+TRANSFORM(aret[1,59],ALLTRIM('@E '+m_set[1,98]))+'-Ped:'+TRANSFORM(mquantd,'99999.99')+'-Aut.:','LIB_SALDO',,'',STRZERO(mcod_merc,5))
                                                DBUNLOCKALL();LOOP
                                        ENDIF
                                ENDIF
                        ENDIF
                                mprod_merc:={}
                                sr_getconnection():exec("SELECT * FROM sacmerc WHERE cod_merc = "+sr_cdbvalue(STRZERO(mcod_merc,5)),,.t.,@mprod_merc)
                                sr_getconnection():exec('COMMIT',,.f.)
				IF LEN(mprod_merc) = 0
				...
				//mlinha := mcod_barra (14)+munidade(3)+saldo(13)+pr_ven(14)
                                mlinha := mprod_merc[1,2]+mprod_merc[1,8]+mprod_merc[1,9]+mprod_merc[1,14]+STRZERO((aret[1,56] - mquantd) * 1000,13)+STRZERO(iat(mprod_merc[1,46])*100,14)+mprod_merc[1,68]
                        ...
                        mconvidado := SPACE(30)
                        IF ! EMPTY(mchass)
                                sr_getconnection():exec("UPDATE merc_det SET venda='V',n_ped="+sr_cdbvalue(STRZERO(mnum_ped,6))+",dat_venda="+sr_cdbvalue(mdata_sis)+",vl_vendido="+sr_cdbvalue(mvlr_fat * 1)+" WHERE  merc_det.codigo = "+sr_cdbvalue(STRZERO(mcod_merc,5))+" AND merc_det.chassis  = "+sr_cdbvalue(mchass)+" AND merc_det.venda IS NULL",,.f.)
                        ENDIF
                        IF mcod_pres > 0
                                cons_presen:={}
                                sr_getconnection():exec("SELECT * FROM sacpresen WHERE codcli = "+sr_cdbvalue(STRZERO(mcod_pres,5))+" AND cod_merc = "+sr_cdbvalue(STRZERO(mcod_merc,5)),,.t.,cons_presen)
                                sr_getconnection():exec('COMMIT',,.f.)
                                IF LEN(cons_presen) > 0
                                        sr_getconnection():exec("UPDATE sacpresen SET qtd ="+sr_cdbvalue(cons_presen[1,6] - mquantd)+" WHERE codcli = "+sr_cdbvalue(STRZERO(mcod_pres,5))+" AND cod_merc = "+sr_cdbvalue(STRZERO(mcod_merc,5)),,.f.)
                                ENDIF
                        ENDIF
                        sr_getconnection():exec("COMMIT",,.f.)
...
FUNCTION fecha_pv(mtip,magrupar,orc_tp)
*****************
...
MEMVAR no,mcod_cli,mnome_ven,m_matriz,m_codigo,m_num,mcod_aux,;
       mnum_ped,mlci,mlib_spc,mprz,mtp_venda,mcond_veze,mdata_sis,m_dia,cod_operado,;
       m_alt,msit_tip,mcont_item,mtela,m_pedido,;
...
mp := ' '
i:=0
IF VAL(m_set[1,84]) > 0 .AND. (mtip = 'SAC23P1' .OR. mtip = '1' .OR. mtip = 'SAC23ORC') .AND. EMPTY(mcod_cli)
        mcod_cli := VAL(m_set[1,84])
ENDIF
mt_pedido := 0
op_tela(10,10,18,70,' Confirmacao de Vendedor ')
WHILE .T.
        muf_trans := SPACE(2)
        mcod_tran := SPACE(4)
...
ENDDO
wvw_lclosewindow()
op_tela(05,10,30,85,' FECHAMENTO DA PRE-VENDA ')
WHILE .T.
        limpa(00,00,33,100)
        IF orc_tp = '*'
        ...
        setcor(1)
        IF m_indiv[1,9] <> 'V'
                DEVPOS(lcia+01,ccia+1);DEVOUT('Sub-Total PRE-VENDA:')
        ...
        ELSE
                ...
                DEVPOS(lcia+21,ccia+1);DEVOUT('Sub-Total PRE-VENDA:')
        ENDIF
...
*/
        NEXT
        IF LEN(m_codigo) = 0
                atencao('Nao foi Solicitacao nenhum PRE-VENDA')
                SET KEY -8 TO
                EXIT
        ENDIF
        i = 1
        ...
        ***
        ...
        mcli_aux := mcod_cli
    IF m_indiv[1,9] <> 'V'
        IF mtip = '*'       //.OR. orc_tp <> NIL
                @ lcia+3,ccia+7 GET mcod_cli PICT '99999'       //WHEN m_set[1,17] = 'S' .AND. m_set[1,100] = 'N' .AND. m_set[1,10] = 'N' .AND. ali <> 'pv'    // men_get(lcia+4,ccia+7,'Informe o Cod.do Cliente - <ENTER>outro campo - <F1>Pesquisar - <F9>p/Incluir Clientes') .AND.          //.AND. m_indiv[1,26] = 'N'
        ELSE
                @ lcia+3,ccia+7 GET mcod_cli PICT '99999' WHEN m_set[1,17] = 'S' .AND. m_set[1,100] = 'N' .AND. m_set[1,10] = 'N'    // men_get(lcia+4,ccia+7,'Informe o Cod.do Cliente - <ENTER>outro campo - <F1>Pesquisar - <F9>p/Incluir Clientes') .AND.          //.AND. m_indiv[1,26] = 'N'
        ENDIF
        ...
        IF LASTKEY() = 27
                IF mtip = '*' .AND. malteracao = 1
                        atencao('Voce nao pode abandonar esta alteracao sem fazer o FECHAMENTO porque este PRE-VENDA ja foi ALTERADO OK')
                        LOOP
                ENDIF
                CLEAR GETS
                SET KEY -8 TO
        ...
        *****************
        ...
        IF ! EMPTY(mnome_cli) .OR. ! EMPTY(mcod_cli) .OR. ! EMPTY(mcpf) .OR. ! EMPTY(mcgc)
                setcor(3)
                DEVPOS(lcia+3,ccia+7);DEVOUT(STRZERO(mcod_cli,5))
                ...
                minsc := cli->insc
                mlimite := cli->limite
                IF STRZERO(mcod_cli,5) <> m_set[1,84] .AND. ! limite(mcod_cli,mt_pedido,'*')
                        UNLOCK
                        LOOP
                ENDIF
                IF ! EMPTY(mdocum)
		...
		mensagem('Preencha os campos')
                ...
                IF ! EMPTY(mplaca)
                        m_envelope:={}
                        sr_getconnection():exec("SELECT * FROM sacpv WHERE penvelope = "+sr_cdbvalue(mplaca),,.t.,@m_envelope)
                        sr_getconnection():exec('COMMIT',,.f.)
                        IF LEN(m_envelope) > 0
    ...
    ENDIF
        ...
        IF m_indiv[1,26] = 'N' .OR. ver_serie() = '141414'         // NAO AUTOCAIXA
                ...
                IF mtp_venda = 'AV' .AND. mtip = '*'
                        IF mforma_pg = 2
                                IF ! aut_sen('Forma de Pagamento estar Diferente da Emissao, Senha autorizacao:','FORMPAGPED',,STRZERO(mcod_cli,5))
                                        LOOP
                                ENDIF
                        ENDIF
                ENDIF
                ...
                DO CASE
                        CASE mforma_pg = 1
                                ...
                                IF modo_pg = 1
                                        ...
                                        /*
                                        IF ver_serie() = '141204'
                                                IF m_set[1,100] = 'N' .AND. ! spc(mcod_cli,mt_pedido,,,mlib_spc)
                                                        UNLOCK
                                                        LOOP
                        ...
                        CASE mforma_pg = 2
                                ...
                                DO CASE
                                        CASE modo_pg = 2
                                                ...
                                                READ
                                                IF m_dia[1] > m_set[1,49] .AND. ! aut_sen('Prazo Permitido '+TRANSFORM(m_set[1,49],'999')+' Solicitado '+STRZERO(m_dia[1],3)+' - Autorizacao:','LIB_PRZ',,STRZERO(mcod_cli,5))
                                                        DBUNLOCK();LOOP
                                                ENDIF
...
*                                                               mvalor_pres := iat((mvalor_aux + mtaxa_fin) * maliq_fin,2)
                                                ...
                                                ELSE
                                                        ...
                                                        IF mvalor_fin < mt_pedido
                                                                atencao('O Valor do financiamento estar menor que o Valor do PRE-VENDA')
                                                                LOOP
                                                        ENDIF
                                                ENDIF
                                                IF mcobra_fin = 'S'
...
*                                                               mcond_int := STRZERO(CTOD(cli->venc2_dup+'/'+STRZERO(VAL(SUBSTR(DTOC(mdata_sis),4,2))+1,2)+'/'+IF(VAL(SUBSTR(DTOC(mdata_sis),7))=12,STRZERO(VAL(SUBSTR(DTOC(mdata_sis),7))+1,2),SUBSTR(DTOC(mdata_sis),7)))-mdata_sis,2)
                                                ...
                                                ENDIF
                                                READ
                                                IF EMPTY(mcod_cond) .AND. m_dia[1] > m_set[1,49] .AND. ! aut_sen('Prazo Permitido '+TRANSFORM(m_set[1,49],'999')+' Solicitado '+STRZERO(m_dia[1],3)+' - Autorizacao:','LIB_PRZ',,STRZERO(mcod_cli,5))
                                                        DBUNLOCK();LOOP
                                                ENDIF
                                                IF VAL(SUBSTR(mcond_veze,1,1)) = 1
                                                        @ lcia+11,ccia+21 GET mvlr_ent PICT '99,999.99' VALID IF(EMPTY(mvlr_ent),.F.,.T.)
			...
			IF mop_desc = '1'
                                ...
                                READ
                                IF mdesc > m_set[1,33] .AND. ! aut_sen('Desconto a maior.. senha de autorizacao:','LIB_DESC',,STRZERO(mcod_cli,5))
                                        LOOP
                                ENDIF
                                mdesc := mdesc/100
				...
				SETCOLOR('i*')
                                ...
                                IF mdesc*100 > m_set[1,33] .AND. ! aut_sen('Desconto a maior.. senha de autorizacao:','LIB_DESC',,STRZERO(mcod_cli,5))
                                        LOOP
                                ENDIF
                                mvlr_desc := mt_pedido * mdesc
                                mt_pedido := mt_pedido - (mt_pedido * mdesc)
        ...
        DEVPOS(lcia+18,ccia+30);DEVOUT('TOTAL VENDA.:')
        setcor(1)
        ...
        ************* DATA DA ENTREGA *****************
        WHILE .T.
                ...
                IF (! EMPTY(m_set[1,162]) .OR. ! EMPTY(m_set[1,163]) .OR. ! EMPTY(m_set[1,164])) .AND. ! EMPTY(mpd_entrega)
                        mped_exp:={}
                        sr_getconnection():exec("SELECT pnum_ped,sum(pquantd*pvlr_fat),sum(pquantd) FROM sacpv WHERE pd_entrega = "+sr_cdbvalue(mpd_entrega)+" AND (ppag = '*' OR ppag IS NULL) GROUP BY pnum_ped",,.t.,@mped_exp)
                        sr_getconnection():exec('COMMIT',,.f.)
        ...
        ENDDO
        *********************************************************
        mopcao := op_simnao('S','Confirma Fechamento do PRE-VENDA [S/n]:')
        IF mopcao = 'N'
                SET KEY -8 TO
                LOOP
        ELSEIF mopcao = 'S'
                ...
                IF mjuros > 1 .OR. ! EMPTY(mdesc)
                        mensagem('Atualizando e Recalculando o PRE-VENDA....')
                        i := 0
                        cons_ped := {}
                        ...
                        FOR i = 1 TO LEN(cons_ped)
                                ...
                                ENDIF
                                ccomm := "UPDATE sacpv SET pcgc = "+sr_cdbvalue(mcgc)
                                ...
                                ccomm := ccomm + ",pcod_cli = "+sr_cdbvalue(STRZERO(mcod_cli,5))
                                ...
                                ccomm := ccomm + ",pproducao = "+sr_cdbvalue('')
                        ...
                        NEXT
                ELSE
                        mensagem('Atualizando PRE-VENDA....')
                        ccomm := "UPDATE sacpv SET pcgc = "+sr_cdbvalue(mcgc)
                        ccomm := ccomm + ",pcpf = "+sr_cdbvalue(mcpf)
                        ccomm := ccomm + ",pplaca = "+sr_cdbvalue(mplaca)
                        ccomm := ccomm + ",pcarro = "+sr_cdbvalue(mcarro)
                ...
                sr_getconnection():exec("UPDATE sacpv SET pfecha = 'F' WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6)),,.f.)

                mensagem('Atualizando o CLIENTE....')
                //cComm  := "UPDATE saccli SET ult_orc="+sr_cdbvalue(mdata_sis)+" WHERE  cod_cli = "+sr_cdbvalue(STRZERO(mcod_cli,5))
                //sr_getconnection():exec(ccomm,,.f.)
                IF ! EMPTY(mnum_os)
                ...
                ENDIF
                sr_getconnection():exec("COMMIT",,.f.)
                //mensagem('Impressao do PRE-VENDA....')
                ...
                m_alt:={}
                SET KEY 28 TO menu_cons()
                op_tela(5,10,11,90,' P R E - V E N D A ')
                botao(0,0,23,99)
                SUB_BANNER(00,01,STRZERO(mnum_ped,10))
                mensagem('Pressione qualquer tecla pra continuar....')
                @ 55,100 GET opcao
                READ
                wvw_lclosewindow()
                m_matriz := {}
...
setcor(1)
c_pedi := {}
sr_getconnection():exec("SELECT * FROM sacpv WHERE pnum_ped = "+sr_cdbvalue(STRZERO(mnum_ped,6)),,.t.,@c_pedi)
sr_getconnection():exec('COMMIT',,.f.)
m_alt := {}
m_codigo := {}
m_posi := {}
...
ENDIF
setcor(10);mponto := ACHOICE(02,01,27,78,m_alt,,,mponto);setcor(1)
wvw_lclosewindow()
IF mponto = 0
        RETURN .T.
ENDIF
...
DO WHILE .T.
        ...
        IF mquantd > malt_merc[1,56] + c_pedi[mponto,14] .AND. m_set[1,107] = 'S'
        ...
        IF mquantd = 0
                ...
                IF opcao = 'S'
                        ...
                        mensagem('Atualizando o Produto...')
                                mprod_merc:={}
                                sr_getconnection():exec("SELECT * FROM sacmerc WHERE cod_merc = "+sr_cdbvalue(m_codigo[mponto]),,.t.,@mprod_merc)
                                sr_getconnection():exec('COMMIT',,.f.)
                                mlinha := mprod_merc[1,2]+mprod_merc[1,8]+mprod_merc[1,9]+mprod_merc[1,14]+STRZERO((mprod_merc[1,56] + c_pedi[mponto,14])*1000,13)+STRZERO(iat(mprod_merc[1,46])*100,14)+mprod_merc[1,68]
                                sr_getconnection():exec("UPDATE sacmerc SET saldo_mer = "+sr_cdbvalue(mprod_merc[1,56] + c_pedi[mponto,14])+",chv_cript = "+sr_cdbvalue(CRIPTO(mlinha,1))+" WHERE cod_merc = "+sr_cdbvalue(m_codigo[mponto]),,.f.)
                                ...
                                sr_getconnection():exec('INSERT INTO logproduto (data_sis,data,'+;
                                                                                ...
                                                                                sr_cdbvalue(mprod_merc[1,56])+','+; //6
                                                                                sr_cdbvalue(mprod_merc[1,56] + c_pedi[mponto,14] )+','+; //7
                                                                                sr_cdbvalue(cod_operado)+','+; //8
                                                                                sr_cdbvalue('SAC23PD1')+','+; //9
                                                                                sr_cdbvalue(LEFT(NETNAME(),15))+','+; //12
                                                                                sr_cdbvalue('EXCLUSAO PV: '+STRZERO(mnum_ped,6))+','+; //11
                                                                                sr_cdbvalue('E')+','+; //11
                                                                                sr_cdbvalue(' ')+')',,.f.)
                        mensagem('Atualizando o Arquivo de PEDIDO...')
                        //sr_getconnection():exec("DELETE FROM sacpv WHERE SR_RECNO = "+sr_cdbvalue(m_posi[mponto]),,.f.)
                        sr_getconnection():exec("UPDATE sacpv SET tipo_ped = "+sr_cdbvalue(1)+" WHERE SR_RECNO = "+sr_cdbvalue(m_posi[mponto]),,.f.)
                        sr_getconnection():exec("COMMIT",,.f.)
                        m_codigo[mponto] = '     '
        ...
        IF opcao = 'S'
                ...
                aret:={}
                        ...
                        sr_getconnection():exec("SELECT * FROM sacmerc WHERE cod_merc = "+sr_cdbvalue(m_codigo[mponto]),,.t.,@mprod_merc)
                        sr_getconnection():exec('COMMIT',,.f.)
                        mlinha := mprod_merc[1,2]+mprod_merc[1,8]+mprod_merc[1,9]+mprod_merc[1,14]+STRZERO((mprod_merc[1,56] - (mquantd - c_pedi[mponto,14]))*1000,13)+STRZERO(iat(mprod_merc[1,46])*100,14)+mprod_merc[1,68]
                        sr_getconnection():exec("UPDATE sacmerc SET saldo_mer = "+sr_cdbvalue(mprod_merc[1,56] - (mquantd - c_pedi[mponto,14]))+",chv_cript = "+sr_cdbvalue(CRIPTO(mlinha,1))+" WHERE cod_merc = "+sr_cdbvalue(m_codigo[mponto]),,.f.)
                        sr_getconnection():exec('INSERT INTO logproduto (data_sis,data,'+;
                                                             'hora,cod_prod,quantd,saldo_ant,saldo_pos,cod_oper,prog,terminal,'+;
                                                             'processo,ent_sai,SR_DELETED )'+;
                                                             ' VALUES ('+;
                                                             ...
                                                             sr_cdbvalue(mprod_merc[1,56])+','+; //6
                                                             sr_cdbvalue(mprod_merc[1,56] - (mquantd - c_pedi[mponto,14]))+','+; //7
                                                             sr_cdbvalue(cod_operado)+','+; //8
                                                             sr_cdbvalue('SAC23PD1')+','+; //9
                                                             sr_cdbvalue(LEFT(NETNAME(),15))+','+; //12
                                                             sr_cdbvalue('ALTERACAO PV: '+STRZERO(mnum_ped,6))+','+; //11
                                                             sr_cdbvalue(IF(mquantd < c_pedi[mponto,14],'E','S'))+','+; //12
                                                             sr_cdbvalue(' ')+')',,.f.)
                cComm  := "UPDATE sacpv SET pquantd = "+sr_cdbvalue(mquantd)
