...
****************************************************
        ...
        *****************
        ...
        Mensagem('Informe o Cod.do CLIENTE - [F1]p/Consultar Cliente - [ALT+R]Consulta de Documentos - [ENTER]p/informa o tipo de DOCUMENTO - <ESC>p/Sair')
        mnota := SPACE(8)
        mcnpjcpf := SPACE(14)
        mcliente := SPACE(40)
        ...
        IF ! EMPTY(mcod_cli)
                setcor(3)
                DEVPOS(lc+1,cc+28);DEVOUT(STR(mcod_cli))
                setcor(1)
                ver_cli(mcod_cli,lc+1,cc+35)
                ver_aniv(mcod_cli)
                IF ver_serie()='141237'
                ...
                ************************************************************
                ...
                ELSE
                        cComm  := "SELECT * FROM saccred WHERE fornec = "+sr_cdbvalue(mcod_cli)
                ENDIF
        ...
        ELSE
                ...
                *****************
                ...
                IF mcod_cli <> mforn_aux
                        GO TOP
                        IF mtp = 'S'
                                IF ! (malias)->(DBSEEK(mtipo+mnumero+mduplicata+'B'+mcod_cli))
                                        atencao('Documento nao Cadastrado ou NAO FOI PAGO !!!')
                                        LOOP
                                ENDIF
                        ELSE
                                IF ! (malias)->(DBSEEK(mtipo+mnumero+mduplicata+' '+mcod_cli))
                ...
                mobs1= (malias)->obs1
                IF ver_serie() = '141304'
                        mdias_atras := mdatpag-((malias)->venc+m_set[1,106])
                ELSE
                        mdias_atras := mdatpag-(malias)->venc
                ENDIF
                IF mdias_atras > 0 .AND. mtp_ = NIL
                        mdias_atras := mdatpag-(malias)->venc
                        mmulta := (malias)->valor * ((m_set[1,50])/100)
                        mjuros := ((malias)->valor * ((mdias_atras*m_set[1,51])/100))
                ENDIF
                mvlpago=(malias)->valor+mjuros+mmulta
                setcor(3)
                @ lc+1,cc+28 SAY mcod_cli
                ...
                IF m_set[1,149] = 'R' .OR. m_set[1,149] = 'A'
                        ...
                        ELSE
                                ...
                                WHILE opcao = 'S'
                                        ...
                                        //DEVPOS(12,11);DEVOUT('  <ENTER> p/Autenticar - <ESC> p/Sair')
                                        //INKEY(0)
