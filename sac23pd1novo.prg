...
* SOLICITAR PEDIDO
*******************
FUNCTION sac23pd1novo(orc,venda,m_altpd)
************************************
...
LOCAL mp_venda,msele,morde,f,mvar,mtelap,mtot_ipi,telaobs,;
      ...
      op_cartao := ' ',npesquisa,nbotao

PRIVATE MPRG:='SAC23PD1NOVO',;
        ...
        mdescri:=SPACE(30),mdesc1:=SPACE(20),cons_cli:={}
...
op_tela(00,00,30,90,memp_resa+SPACE(50)+'EMISSAO DE PEDIDOS'+SPACE(40)+'Terminal: '+m_indiv[1,1]+'  ['+mprg+']',,1)
...
WHILE .T.
        ...
        WHILE .T.
                ...
                IF m_indiv[1,26] = 'T'
                        ...
                        mflag := 1
                        op_tela(00,01,31,90,' A B E R T U R A   D O   P E D I D O                      Informacoes para o PEDIDO ** 1 ** ',,1)
                        botao(0,0,7,120)
                ...
                IF m_indiv[1,9] <> 'V' .AND. m_indiv[1,26] # 'P'
					...
					mpt_os := RECNO()
                        ...
                        IF m_set[1,10] = 'S' .AND. EMPTY(mnum_ped)
                                WHILE .T.
                                        SET KEY -8 TO sac130()           // inclusao de clientes
                                        IF ! AbriArq('saccli','cli');RETURN NIL;ENDIF
                        ...
                        WHILE .T. .AND. m_set[1,108] = 'S' .AND. EMPTY(mnum_ped)
                        ...
                        IF ver_nivel(mprg+'AV','EMISSAO DE PEDIDO VENDA DE AVARIAS','1',nivel_acess,'*') .AND. (ver_serie() = '141225' .OR. ver_serie() = '141254')
                                ...
                                ENDIF
                        ENDIF
                        IF ! EMPTY(cons_cli[1,59]) .AND. op_cartao = 'S'
                                ...
                                FOR y = 1 TO LEN(c_orcam)
                                        ...
                                        */
                                        ...
                                        ELSEIF mped_merc[1,103] = 'S' .AND. c_orcam[y,14] > mped_merc[1,56]
                        	                ...
                        	                LOOP
                                        ELSEIF c_orcam[y,14] > mped_merc[1,56] .AND. m_set[1,107] = 'S' .AND. SUBSTR(mped_merc[1,9],1,1) <> '@'
                	                        IF ! aut_sen('Quantidade Solicitada: '+ALLTRIM(TRANSFORM(c_orcam[y,14],'999,999,999.99'))+' maior que saldo: '+ALLTRIM(TRANSFORM(mped_merc[1,56],'999,999.99'))+' ... Senha de autorizacao:','LIB_SALDO',,,c_orcam[y,6])
                        	                        LOOP
                                	        ENDIF
                                        ELSEIF c_orcam[y,14] > mped_merc[1,56] .AND. mped_merc[1,9] <> 'DIVERSOS' .AND. m_set[1,107] = 'S' .AND. SUBSTR(mped_merc[1,9],1,1) <> '@'
                                                IF ! aut_sen('Quantidade Solicitada: '+ALLTRIM(TRANSFORM(c_orcam[y,14],'999,999,999.99'))+' maior que saldo: '+ALLTRIM(TRANSFORM(mped_merc[1,56],'999,999.99'))+' ... Senha de autorizacao:','LIB_SALDO',,,c_orcam[y,6])
                                                        LOOP
                                                ENDIF
                                                mlibera := 'S'
...
*/
		...
		ENDIF
                ...
                ELSEIF m_indiv[1,26] <> 'T' .AND. m_indiv[1,26] <> 'P' .OR. (mped_merc[1,25] = 'S' .OR. mvlr_fat = 0)
                        ...
                        ENDIF
                        // FRIPAL, FRANGO DOURADO ou DISCAPE
                        IF mvlr_fat < (mped_merc[1,45] * 1.10) .AND. (ver_serie() = '141597' .OR. ver_serie() = '141390' .OR. ver_serie() = '141336')
